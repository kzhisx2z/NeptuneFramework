function Neptune.Game.SpawnVehicle(model, coords, heading, callback)
    ...
    CreateThread(function()
        ...
        Neptune.Streaming.RequestCollision(vehicle, vectorCoords)
...
function Neptune.Game.GetVehicles()
    return GetGamePool('CVehicle')
end

function Neptune.Game.GetPlayers(onlyOtherPlayers, returnKeyValue, returnPeds)
    local players = {}
    ...
    for _, player in ipairs(GetActivePlayers()) do
        ...
        if DoesEntityExist(ped) and (onlyOtherPlayers and player ~= selfPlayer or not onlyOtherPlayers) then
            players[(returnKeyValue and player or #players + 1)] = returnKeyValue and ped or (returnPeds and ped or player)
        end
    end
...
function Neptune.Game.GetClosestEntity(entities, isPlayerEntities, coords, filter)
    ...
    if filter then
        ...
        for k, entity in pairs(entities) do
            if filter[GetEntityModel(entity)] then
    ...
    for k, entity in pairs(filteredEntities or entities) do
        ...
        if closestEntityDistance == -1 or distance < closestEntityDistance then
            closestEntity = isPlayerEntities and k or entity
...
function Neptune.Game.GetClosestVehicle(coords, filter)
    return Neptune.Game.GetClosestEntity(Neptune.Game.GetVehicles(), false, coords, filter)
end

function Neptune.Game.GetClosestPlayer(coords)
    return Neptune.Game.GetClosestEntity(Neptune.Game.GetPlayers(true, true), true, coords)
end

function Neptune.Game.GetVehicleProperties(vehicle)
...
function Neptune.Game.SetVehicleProperties(vehicle, props)
    if not DoesEntityExist(vehicle) then
        return
    end
    local colorPrimary, colorSecondary = GetVehicleColours(vehicle)
